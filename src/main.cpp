...
WhoopController controller1(joystickmode::joystickmode_split_arcade,
                            controllertype::controller_primary);
...
WhoopDriveOdomUnit odom_unit(
    1.51_in,   // The forward tracker distance from the odom unit's center.
               // (positive implies a shift to the right from the odom unit's
               // center)
    2.5189_in, // Diameter of the forward tracker (e.g., 3.25_in for 3.25-inch
               // wheels).
    -4.468_in, // The sideways tracker distance from the odom unit's center
               // (positive implies a shift forward from the odom unit center)
    2.5189_in, // Diameter of the sideways tracker (e.g., 3.25_in for 3.25-inch
               // wheels).
    &inertial_sensor, // Pointer to the WhoopInertial sensor
    &forward_tracker, // Pointer to the forward tracker, as a WhoopRotation
                      // sensor
    &sideways_tracker // Pointer to the sideways tracker, as a WhoopRotation
                      // sensor
);

WhoopDriveOdomOffset odom_offset(
    &odom_unit, // Pointer to the odometry unit (will manage the odom unit)
    -0.6_in,    // The x offset of the odom unit from the center of the robot
             // (positive implies a shift right from the center of the robot).
    4.95_in // The y offset of the odom unit from the center of the robot
...
BufferNode buffer_system(
    256, // The buffer size, in characters. Increase if necessary, but at the
         // cost of computational efficiency.
    debugmode::debug_disabled // debugMode::debug_disabled for competition use,
                              // debugMode::debug_enabled to allow the code to
                              // pass errors through
);
...
WhoopVision vision_system(
    ...
    &buffer_system, // Pointer to the buffer system (will be managed by the
                    // buffer system)
    "P" // The subscribed stream name to receive the pose from the Jetson Nano
);

// This is the jetson commander. It sends keep-alive messages intermittently and
// also commands the Jetson Nano. This is essential to ensure that the nano
// starts its internal program, stop program, restarts program, and can be told
...
JetsonCommander jetson_commander(
    ...
    60_sec, // In seconds. When the V5 Brain shuts down or disconnects, the
            // Jetson Nano will keep the program running for this time before
            // it shuts off
    2_sec,  // How many seconds to wait before sending anoter keep alive message
            // to Jetson (suggested 2)
    jetsonCommunication::disable_comms // If you don't have a Vision Tesseract
                                       // on your robot, set to disable_comms
);
...
WhoopOdomFusion odom_fusion(
    ...
    50_in,  // If FusionMode is fusion_gradual, it is the maximum allowable
            // lateral shift the vision camera can update per second.
    500_deg // If FusionMode is fusion_gradual, it is the maximum allowable yaw
            // rotational shift the vision camera can update per second.
);
...
PursuitParams pursuit_parameters(
    ...
    50.0_v,  // The maximum voltage change per second, as a slew rate (only
             // applies speeding up)
    ...
    0_sec,   // Time after which to give up and move on, in seconds (set to 0 to
             // disable)
    ...
    100      // The number of points when generating the path. More points mean
             // higher detail of the path, but at a higher computational cost
);
...
WhoopDrivetrain robot_drivetrain(
    &pursuit_parameters,  // The default pure pursuit parameters for operating
                          // the robot in autonomous
    &odom_fusion,         // Odometry fusion module
    PoseUnits::in_deg_cw, // Set default pose units if not defined. "m_deg_cw"
                          // means "meters, degrees, clockwise-positive yaw",
                          // "in_deg_ccw" means "inches, degrees,
                          // counter-clockwise-positive yaw", and so forth.
    &controller1,         // Pointer to the controller
    &left_motors, // Pointer to the left motor group (optionally can be a list
                  // of motors as well)
    &right_motors // Pointer to the right motor group (optionally can be a list
                  // of motors as well)
);
 ...
 */
void disabled() { robot_drivetrain.set_state(drivetrainState::mode_disabled); }
...
void opcontrol() {
  ...
  while (true) {
    ...
    pros::lcd::print(3, "FO (%s): %.1f %.1f %.1f %.1f %.1f %.1f",
                     robot_drivetrain.get_units_str().c_str(), current_pose.x,
                     current_pose.y, current_pose.z, current_pose.pitch,
                     current_pose.yaw, current_pose.roll);
